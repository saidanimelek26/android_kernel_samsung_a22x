...
static struct sg_table *cma_heap_map_dma_buf(
		struct dma_buf_attachment *attachment,
		enum dma_data_direction direction)
{
	struct dma_heap_attachment *a = attachment->priv;
...
static void *cma_heap_map_atomic(struct dma_buf *dmabuf, unsigned long page_num)
{
	struct cma_heap_buffer *buffer = dmabuf->priv;

	if (page_num >= buffer->pagecount)
		return NULL;

	return kmap_atomic(buffer->pages[page_num]);
}

static void cma_heap_unmap_atomic(struct dma_buf *dmabuf,
				  unsigned long page_num, void *vaddr)
{
	kunmap_atomic(vaddr);
}

static void *cma_heap_map(struct dma_buf *dmabuf, unsigned long page_num)
{
	struct cma_heap_buffer *buffer = dmabuf->priv;
...
static void cma_heap_unmap(struct dma_buf *dmabuf, unsigned long page_num,
			   void *vaddr)
{
	struct cma_heap_buffer *buffer = dmabuf->priv;
...
static int __add_cma_heap(struct cma *cma, void *data)
{
	...
	exp_info.name = name ? name : "default_cma_region";
