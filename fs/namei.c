...
static int nd_jump_root(struct nameidata *nd)
{
	...
	if (unlikely(nd->flags & LOOKUP_NO_XDEV) &&
	    nd->path.mnt && nd->path.mnt != nd->root.mnt)
		return -EXDEV;
	if (!nd->root.mnt) {
		...
		const struct open_flags *op)
{
	...
	if (unlikely(filp == ERR_PTR(-ECHILD)) &&
	    unlikely(flags & LOOKUP_CACHED))
		filp = ERR_PTR(-EAGAIN);
	else if (unlikely(filp == ERR_PTR(-ECHILD)))
...
{
	return do_linkat(olddfd,
			 getname_flags(oldname,
...
retry:
	from = filename_parentat(olddfd, from, lookup_flags,
	...
	to = filename_parentat(newdfd, to, lookup_flags,
				&new_path, &new_last, &new_type);
	if (IS_ERR(to)) {
		error = PTR_ERR(to);
		goto out_put_from_path;
	}
